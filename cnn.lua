...
for loopIterator=1, trainingCycles, 1
do
	print("Training loop:", loopIterator)

	-- get input, output frames (first, last, middle)
	input_frames[1][1] = trainingFrames[loopIterator] -- first
	input_frames[1][2] = trainingFrames[loopIterator + 2] -- last
	output_frames_middle[1] = trainingFrames[loopIterator + 1] -- middle
...
do
	...
	groundTruthFrame = torch.Tensor(batch_size, n_channels, frame_height, frame_width)
	groundTruthFrame[1] = testingFrames[loopIterator+1]
