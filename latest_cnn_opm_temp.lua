...
allFrames = load_images.load('opmFrames', frameCount)
...
sizeOfTrainingSet = math.floor((32/100) * numberOfInputFramesToUse)
...
do
	...
	-- predict new frames
	input_frames[1] = testingFrames[loopIterator] -- first
	input_frames[2] = testingFrames[loopIterator +1] -- next
	model:forward(input_frames)
	frameToInsert = model.output
	...
	filename = "OriginalO" .. tostring(loopIterator) .. ".jpg"
	image.save(filename, tempGroundTruth)

	filename = "PredictedO" .. tostring(loopIterator) .. ".jpg"
	image.save(filename, tempPredictedImage)

	filename = "NewO" .. tostring(loopIterator) .. ".jpg"
	image.save(filename, tempInsert)
end
